...
RC RelationManager::deleteTable(const string &tableName)
{
    ...
    // cout << "destroy file rc: " << rc << endl;
    if(rc != 0) return -1;
    
    rc = _rbfm->openFile("tables.tbl", fh);
    // cout << "open file rc: " << rc << endl;
    if(rc != 0) return -1;
    ...
    // cout << "find table rc: " << rc << endl;
    if (rc == RM_EOF) return -1;
    ...
    // cout << "delete reckord rc: " << test << endl;
    // cout << "table rid " << rid.pageNum << '.' << rid.slotNum << endl; 

    rc = _rbfm->closeFile(fh);
    // cout << "close table file rc: " << rc << endl;
    if(rc != 0) return -1;
    
    rc = _rbfm->openFile("columns.tbl", fh);
    // cout << "open col file rc: " << rc << endl;
    if(rc != 0) return -1;
    ...
    while(rmsi->getNextTuple(rid, returnedData) != RM_EOF) {
        // cout << "col rid " << rid.pageNum << '.' << rid.slotNum << endl; 
        // rc = _rbfm->deleteRecord(fh, columnsColumns(), rid);
      ...
      RM_ScanIterator &rm_ScanIterator)
{
    ...
    // cout << "Searching for value str: " << (char*) value << " int: " <<  *(int* )value << endl;
